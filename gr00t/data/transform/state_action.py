    ...
    def __init__(self, from_rep="axis_angle", to_rep="rotation_6d"):
        ...
        if from_rep != "matrix":
            if from_rep == "quaternion":
                funcs = [quaternion_to_matrix, matrix_to_quaternion]
            else:
        ...
        if to_rep != "matrix":
            if to_rep == "rotation_6d":
                funcs = [matrix_to_rotation_6d, rotation_6d_to_matrix]
            else:
                raise NotImplementedError
                import pytorch3d.transforms as pt
    ...
    def validate_modality_metadata(cls, v):
        for modality_key, config in v.items():
            if isinstance(config, dict):
                ...
                assert isinstance(config, StateActionMetadata), (
    ...
    def set_metadata(self, dataset_metadata: DatasetMetadata):
        ...
        for key in self.apply_to:
            ...
            if key not in self.modality_metadata:
                ...
                assert hasattr(modality_metadata, modality), (
                    f"{modality} config not found"
                )
                assert state_key in getattr(modality_metadata, modality), (
        ...
        for key in self.normalization_modes:
            ...
            modality, state_key = split_key
            assert hasattr(dataset_statistics, modality), (
                f"{modality} statistics not found"
            )
            assert state_key in getattr(dataset_statistics, modality), (
