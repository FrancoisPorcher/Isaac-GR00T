...
def node_eval(
    ...
    print(
        f"[Node {node_id}] hostname={hostname} "
    ...
    try:
        gpu_check = subprocess.run(
            [
                "nvidia-smi",
                "--query-gpu=index,name,memory.total,driver_version,ecc.errors.uncorrected.volatile.total",
                "--format=csv,noheader",
            ],
            capture_output=True,
            text=True,
    ...
    with ThreadPoolExecutor(max_workers=gpus_per_node) as pool:
        futures = [
            ...
            for gpu_id, tasks in enumerate(my_partitions)
            if tasks
        ]
        for f in futures:
            gpu_id, rc = f.result()
...
def main():
    ...
    parser.add_argument(
        "--split", type=str, default="target", choices=["pretrain", "target"]
    )
    parser.add_argument(
        "--task_set",
        type=str,
    ...
    print(
        f"Tasks: {len(tasks)}, Nodes: {args.n_nodes}, GPUs: {n_gpus}, Episodes: {args.n_episodes}"
    )
